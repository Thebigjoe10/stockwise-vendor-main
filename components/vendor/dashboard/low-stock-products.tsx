...
type LowStockProductsProps =
  | { data: Product[] }                                   // you pass an array
  | { data: { lowStockProducts: Product[] } };            // or an object with the array inside

const LowStockProducts = (props: LowStockProductsProps) => {
  // 1) Normalize `data` into a plain array
  const products: Product[] = Array.isArray((props as any).data)
    ? ((props as any).data as Product[])
    : ((props as any).data?.lowStockProducts ?? []);

  // 2) Build rows WITHOUT flatMap
    ...
    for (const product of products) {
      ...
      for (const sub of subs) {
        ...
        for (const s of sizes) {
          if (s && typeof s.qty === "number" && s.qty < 2) {
            rows.push({
              id: s._id,
  ...
  const filteredData = useMemo(() => {
    ...
    return lowStockData.filter((item) => item.productName.toLowerCase().includes(q));
  }, [lowStockData, search]);
